...
| G14 Source and capability review | BLOCKED | partial direct inspection | `CLAIM-REVIEW.json` deduplicates 932 documented claims: 136 retain prior evidence, 3 are corrected and directly reviewed, 1 is partially reviewed and 792 still require claim-level closeout. This is a work inventory, not an assertion that 792 sources are inaccessible. The URL inventory now records 129 body reads, 4 package-source reads, 83 earlier-release URLs, 324 search-only, 55 unconfirmed and 17 without a method; URL counts cannot pass this gate. `CASE-CROSSWALK-REVIEW.json` records four directly reviewed cases and nine crosswalk entries, with four session pages and one catalog page unresolved. |
