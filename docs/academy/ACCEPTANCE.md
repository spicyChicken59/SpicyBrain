...
| G14 Source and capability review | BLOCKED | executed + inspected | [`SOURCE-REVIEW.md`](SOURCE-REVIEW.md) and [`SOURCE-INVENTORY.json`](SOURCE-INVENTORY.json) record how each URL was checked, kept separate from availability. Of 612 URLs: 113 had a file or page body read in this build (source repositories, package indexes, API discovery documents), 4 were read from a package's source, 83 rely on the previous release's review, 341 are confirmed at search level only, and 54 are marked unconfirmed, with the claim hedged or dropped. For 17 URLs no record states how they were checked; the 51 new-module records that state no check are listed in the review. Documentation hosts were blocked, so no page body there was read. CI's probe (run 35871598783 at `cb733cd`, rebuilt from its log with `npm run report:sources -- --from-log <log> --run <url> --head <sha>`) found 615 of 616 URLs answering; the one 403 is a case-analysis blog post. Unknown is recorded as unknown, never as unsupported. |
