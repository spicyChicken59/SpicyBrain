...
| G14 Source and capability review | BLOCKED | partial direct inspection | `CLAIM-REVIEW.json` deduplicates 932 documented claims: 136 retain prior evidence, 8 are corrected and directly reviewed, 18 are directly reviewed and 770 still require claim-level closeout. This is a work inventory, not an assertion that 770 sources are inaccessible. The URL inventory now records 148 body reads, 4 package-source reads, 83 earlier-release URLs, 305 search-only, 55 unconfirmed and 17 without a method; URL counts cannot pass this gate. `CASE-CROSSWALK-REVIEW.json` records four directly reviewed cases and nine crosswalk entries, with four session pages and one catalog page unresolved. |
