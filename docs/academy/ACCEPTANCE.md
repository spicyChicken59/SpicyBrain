...
| G07 Handbook usefulness | PASS | executed | The e2e suite and the collections fixture cover anchors, independent panel scrolling, new-tab context, mobile return, the per-track handbook and print. Handbook depth was reviewed in the editorial reviews. |
| G08 Assessment correctness | PASS | executed | Deterministic answer checks, unanswered, changed and double-submitted answers, and revision snapshots, in the unit and e2e suites. Every check's rationale was reviewed per track; changed assessed meaning moved revisions (G03). |
| G09 Cards and scheduling | PASS | executed | Every module has at least 10 core and exactly 4 extension cards, with no duplicate prompt anywhere in the course (`academy-check`). Filters, detours, interrupted reviews, revised cards and caps are covered in the e2e suite. |
| G10 Media evidence and privacy | PASS | executed + inspected | [`MEDIA-REVIEW.md`](MEDIA-REVIEW.md): the 16 reviewed placements are retained, every new module records a no-placement decision, and 43 candidate leads stay unreviewed and unplaced. No provider request is made before consent (`tests/browser/teacher-first.spec.ts`). No new video was watched and none is claimed. |
| G11 Real local labs | PASS | executed | [`LABS.json`](LABS.json): 24 labs, 20 of them local-executed with 619 tests, run in their recorded environments. CI's `labs` job rebuilds each environment from exact pins, re-executes every lab from its committed ZIP, and compares the result with the committed evidence. The original exercise ZIP is extracted and its advertised commands run in CI. The tabletop labs (L14, L15) and platform guides (L16, L23) record their platform steps as not executed. |
...
| G13 Field guides and capstones | PASS | executed + inspected | Builder review of the 32 guides, 8 cases, the crosswalk and 3 capstones, with fixes applied. `tests/browser/capstones.spec.ts` checks, for each capstone: the revision notice, the data pack bytes against the manifest, a draft kept across a reload, the disclosures, the model answer and a recorded self-assessment. `tests/browser/academy-pages.spec.ts` checks every lab, guide, case and capstone page at 390 px and at 320 px with doubled text. |
| G14 Source and capability review | PASS | executed + inspected | [`SOURCE-REVIEW.md`](SOURCE-REVIEW.md) and [`SOURCE-INVENTORY.json`](SOURCE-INVENTORY.json) record how each URL was checked, kept separate from availability. Of 612 URLs: 113 had a file or page body read in this build (source repositories, package indexes, API discovery documents), 4 were read from a package's source, 83 rely on the previous release's review, 341 are confirmed at search level only, and 54 are marked unconfirmed, with the claim hedged or dropped. For 17 URLs no record states how they were checked; the 51 new-module records that state no check are listed in the review. Documentation hosts were blocked, so no page body there was read. CI's probe (run 35871598783 at `cb733cd`, rebuilt from its log with `npm run report:sources -- --from-log <log> --run <url> --head <sha>`) found 615 of 616 URLs answering; the one 403 is a case-analysis blog post. Unknown is recorded as unknown, never as unsupported. |
...
| G23 Exact-head CI | PASS | executed | CI run [35874825748](https://github.com/spicyChicken59/SpicyBrain/actions/runs/35874825748) on `09cba75`, the last content and code commit, was green in both jobs. `acceptance`: a clean install, `npm run check`, the nested build, the full browser suite on Chromium and WebKit, the content-extension proof, the collections fixture, the source probe, the authored exercises and the original exercise ZIP extracted and executed. `labs`: every download matched to its source, then each lab re-executed from its ZIP in environments built from exact pins and compared with the committed evidence. The same gates passed locally on an isolated copy of that commit. Later commits add only documents and the release manifest; their own exact-head run is recorded on the pull request. |
| G24 Editorial challenge | PASS | inspected | [`EDITORIAL-REVIEW.md`](EDITORIAL-REVIEW.md): a builder review of each track with altered examples, one per guide set, the cases and crosswalk, and the capstones. Every high and medium finding was reproduced before it was fixed, and rejections are recorded with reasons. |
| G25 Release package and privacy | PASS | executed | [`RELEASE.md`](RELEASE.md) and [`release-manifest.json`](release-manifest.json). `tests/public-safety.test.ts` scans content, every academy document, the evidence and the README for build-machine paths and proxy settings. No learner biography, job title, employer relationship, level, compensation or diagnostic information appears in any artefact. |
...
- **Documentation and video hosts were blocked** from the build environment.
  Sources are labelled by how they were checked, CI's reachability probe is
  recorded separately, and no new video was watched.
