...
Current Databricks pages title the workspace coding and data assistant **Genie Code**: it generates and runs code, builds pipelines and AI/BI dashboards, debugs errors and works with Unity Catalog data, in notebooks, the SQL editor, jobs, AI/BI dashboards and the file editor. Former **Databricks Assistant** FAQ, usage and tips addresses now redirect to Genie Code pages. Check the destination and date; some retrieved Google Cloud snapshots are older than the current AWS pages. **Genie Agents**, formerly Genie spaces, are a different feature for business users' plain-English questions.
...
Documentation follow-up (24 September 2026): current AWS Agent mode supports configurable approvals and defaults to Auto-approve on first use. Choose the approval setting before starting; it is not a security boundary. The partner-powered setting has a dated removal timeline, recorded with the modes, billing and SQL sections in the claim ledger.
