...
Model Serving hosts the endpoints behind all of them on Databricks-managed serverless compute, though Databricks developer documentation now recommends running a custom agent itself in a Databricks App; the AI Gateway governs endpoint traffic; MLflow traces and evaluates. That documentation also uses Agent Bricks as the name of its whole agent platform, custom Python agents included; this table's Agent Bricks row means the configured agents. The same model can sit behind every row, so a model's name never decides the block.
...
A serving endpoint exposes a model or agent as a REST service on compute Databricks manages outside your cloud account; callers need CAN_QUERY. For a custom agent on the legacy path, which Databricks developer documentation now says to replace by running the agent in a Databricks App, the agent framework's `deploy()` puts a Unity Catalog model version on an agent endpoint, enables its inference table, routes all traffic to the new version and keeps earlier versions served at zero traffic until deleted.
...
The Agent Bricks agents in this lesson are configured, not coded. A **Knowledge Assistant** takes a description, optional instructions, examples with guidelines and knowledge sources: a vector search index, files in a Unity Catalog volume, or a table of file content. It reports an agent endpoint, an MLflow experiment and a state; files and file-table sources must be synced before new documents are known, while index sources follow their index. A **Supervisor Agent** routes each request among configured tools, such as Knowledge Assistants, Genie Agents (formerly Genie spaces), Unity Catalog functions, MCP connections and apps: autonomous tool selection, which answers more questions and adds wrong-tool and denied-tool failures. The Terraform provider v1.133.0 labels both resources Public Beta; that label does not say whether your region or workspace has them.
<!-- section:dbxfe-ai-platform-l01-custom -->

A custom agent is code: MLflow's `ResponsesAgent` wraps logic written with any agent framework. Databricks developer documentation now recommends running it in a Databricks App and calls the path below, deploying it with `agents.deploy()` to its own serving endpoint after logging it and registering it in Unity Catalog, a legacy path.
...
- Databricks developer documentation source (databricks/devhub at commit 35f4745, 2026-09-21), read in this build on 2026-09-25: the custom agent endpoints, Agent Bricks overview and Genie Agents pages, for the legacy status of `agents.deploy()`, the scope of the Agent Bricks name and the Genie Agent name.
