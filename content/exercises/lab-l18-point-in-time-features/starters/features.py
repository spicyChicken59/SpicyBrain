"""lab-l18-point-in-time-features: STARTER (six marked gaps).

Fill GAP 1 to GAP 6 in order; TASKS.md says what each must return and which
fixture rows prove it. Everything else is complete and matches the reference
solution, so the error messages and the JSON output format are the same.

A feature row carries two times:
...
the row's prediction time. Late readings never rewrite a published row; they
add a new version with a later ``available_at``.

Local pandas 3.0.6 and numpy 2.5.3 on CPython 3.12; fictional machines.

    python starters/features.py            # works once the gaps are filled
"""
...
def refuse_duplicates(frame: pd.DataFrame, keys: list[str], what: str) -> None:
    ...
    raise NotImplementedError("GAP 3 (refuse_duplicates): raise DuplicateEntityTimeError when two rows share the same values in `keys`; the message must start 'duplicate entity-time <what>:' and name the key values and how many rows share them.")
...
def window_end(observed_at: pd.Series, window_hours: int) -> pd.Series:
    ...
    raise NotImplementedError("GAP 1 (window_end): return the END of the two-hour window (start, end] that contains each reading. A reading at exactly 10:00 belongs to the window ending 10:00; one at 10:01 to the window ending 12:00. Hint: one Series.dt method does this on the UTC grid.")
...
def compute_features(observations: pd.DataFrame, run_at: pd.Timestamp, contract: dict) -> pd.DataFrame:
    ...
    raise NotImplementedError("GAP 2 (compute_features): keep readings ingested at or before run_at, assign window ends, keep windows that ended by run_at, then one row per (machine_id, feature_time) with the five FEATURE_COLUMNS and source_max_ingested_at. vib_max_2h must skip missing values, vib_count_2h counts only non-missing values, reading_count_2h counts rows. available_at is run_at. Return FEATURE_TABLE_COLUMNS sorted by machine_id, feature_time.")
...
def point_in_time_join(labels: pd.DataFrame, features: pd.DataFrame, contract: dict) -> pd.DataFrame:
    ...
    raise NotImplementedError("GAP 4 (point_in_time_join): for each label row, among feature rows of the same machine with available_at <= prediction_time, take the newest feature_time and, of that window, the newest available_at. Labels with no such row keep null feature columns. Validate both inputs first, restore types with _typed and call assert_no_future_information before returning.")
...
def assess(prediction_time: pd.Timestamp, feature_time, max_age_minutes: int) -> tuple[int | None, str]:
    ...
    raise NotImplementedError("GAP 5 (assess): return (age_minutes, status): (None, 'none_available') when there is no feature_time; otherwise the whole minutes between feature_time and prediction_time, and 'stale' only when that age is strictly greater than max_age_minutes, else 'ok'.")
...
def online_store_at(features: pd.DataFrame, now: pd.Timestamp) -> pd.DataFrame:
    ...
    raise NotImplementedError("GAP 6 (online_store_at): rows published at or before `now`; per machine keep the newest feature_time and then the newest available_at; index the result by machine_id. A republished older window must not replace a newer one.")
