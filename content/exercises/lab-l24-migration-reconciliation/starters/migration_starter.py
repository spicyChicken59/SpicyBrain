"""Lab L24 starter: the reference module with six deliberate gaps (see TASKS.md).
...
def business_day(rules: Rules, plant_code: str):
    ...
    # Task 4: Return the business-day column for rules.day_rule: server_local wall clock minus the cutoff, plant_local via convert_timezone, or session (an instant rendered in the session zone).
    raise NotImplementedError("Task 4: business_day is yours to write")


def normalize(column, key_rule: str):
    # Task 3: Return the join key for a code under key_rule: 'collation' keeps leading spaces, drops trailing spaces and ignores case; 'binary' compares as stored; 'trim' is the tempting wrong rule.
    raise NotImplementedError("Task 3: normalize is yours to write")


def assign_shift(rows: DataFrame, inspectors: DataFrame, plant_code: str, rules: Rules) -> DataFrame:
    # Task 3: LEFT JOIN the inspectors on the normalized code AND the plant; decide where the plant predicate lives (ON or WHERE) from rules.join_rule, then COALESCE the shift to UNASSIGNED.
    raise NotImplementedError("Task 3: assign_shift is yours to write")


def aggregate(detail: DataFrame, rules: Rules) -> DataFrame:
    # Task 5: Group by plant, business day and shift; count, sum units and money; compute defect_pct with integer division when rules.division == 'integer'.
    raise NotImplementedError("Task 5: aggregate is yours to write")
...
def affected_days(spark, source_rows, plant_code, since, until, select_by="change", rules=TARGET) -> list[list[str]]:
    ...
    # Task 7: Select rows changed in the half-open window (since, until] (or by event time when select_by == 'event') and return the sorted distinct [plant_code, business_day] pairs.
    raise NotImplementedError("Task 7: affected_days is yours to write")
...
def cutover_gate(scenario: dict, required_clean_days: int) -> dict:
    ...
    # Task 8: Count consecutive clean days since the last code change; an 'explained' day counts only when every exception has an owner; require a rehearsed rollback.
    raise NotImplementedError("Task 8: cutover_gate is yours to write")
