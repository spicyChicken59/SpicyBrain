-- Lab L11 STARTER: fill every gap marked "-- GAP" (see TASKS.md). The runner
-- tests this file with:  python run_tests.py --sql starters/model.sql
-- Until the gaps are filled the tests fail; that is expected. Spark SQL 4.0.4.
--
...
WITH resolved AS (
  ...
  LEFT JOIN dim_line l
    ON l.line_id = s.line_id
   -- GAP (task 1): add the validity-window condition so each inspection meets
   -- at most ONE version of its line (valid_from inclusive, valid_to exclusive,
   -- NULL valid_to = still valid).
)
SELECT r.inspection_id,
       r.line_sk AS line_sk,  -- GAP (task 1): a line with no valid version must map to :unknown_line_sk
       r.line_id AS source_line_id,
       r.shift_id,
       r.inspected_at,
       r.business_date,
       r.inspected_qty AS inspected_pieces,   -- GAP (task 2): convert to pieces
       r.defective_qty AS defective_pieces,   -- GAP (task 2): convert to pieces
       r.uom AS source_uom
FROM resolved r
...
grid AS (
  ...
  -- GAP (task 3): also include every observed (line_sk, business_date) from facts,
  -- so no fact can fall outside the grid
)
SELECT g.business_date, l.line_id, l.plant_id, l.supervisor,
       ...
       'ok' AS status,   -- GAP (task 3): :empty_status when no inspection, :zero_status when 0 pieces
       CAST(COALESCE(f.defective_pieces, 0) AS DOUBLE) / COALESCE(f.inspected_pieces, 0) AS rate
       -- GAP (task 3): the rate must be NULL, never an error or 0, when no piece was inspected
FROM grid g
...
facts AS (
  ...
  -- GAP (task 4): check this sums the PARTS; a plant rate is never AVG(line rate)
),
grid AS (
...
FROM (
  ...
  FROM dim_date
  WHERE date <= :as_of   -- GAP (task 5): count only the plant calendar's business days
)
