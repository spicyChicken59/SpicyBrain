"""Reference policy gate for Lab L22 (tool calls, identity and action authorization).

Standard library only. No model runs here: the lab replays scripted tool
proposals (fixtures/calls.json) through an application-side gate. The gate
decides from the authenticated session, the application's exposed tools,
the tool's schema, the user's grants intersected with the application's
delegated scopes, idempotency keys and approval policy. Retrieved content is
copied into the audit as provenance and is never read to make a decision.
Every effect is an append to an in-memory list held by the Downstream stub;
nothing is sent, bought or changed outside this process.
"""
    ...
    for name in sorted(arguments):
        ...
        if rule.get("type") == "string":
            ...
            if "maxLength" in rule and len(value) > rule["maxLength"]:
                problems.append(f"'{name}' longer than {rule['maxLength']} characters")
            ...
            if "minimum" in rule and value < rule["minimum"]:
                problems.append(f"'{name}' {value} below minimum {rule['minimum']}")
            if "maximum" in rule and value > rule["maximum"]:
                problems.append(f"'{name}' {value} above maximum {rule['maximum']}")
        if "enum" in rule and value not in rule["enum"]:
            problems.append(f"'{name}' not one of {rule['enum']}")
    return problems
...
class Gate:
    ...
    def _record(self, at, base, decision, reason, detail="", *, effect_id=None, approval_id=None,
        ...
        row = {"seq": len(self._audit) + 1, "at": at, "event": base["event"], "ref": base["ref"],
               "actor": base["actor"], "on_behalf_of": base["on_behalf_of"],
               "executing_identity": self.application["id"], "tool": base["tool"],
               "args_digest": base["args_digest"], "idempotency_key": base["idempotency_key"],
               "decision": decision, "reason": reason, "detail": detail, "attempts": attempts,
               "effect_id": effect_id, "approval_id": approval_id,
               "influenced_by": list(base["influenced_by"])}
        self._audit.append(dict(row))
        outcome = dict(row)
    ...
    def _needs_approval(self, tool: dict, arguments: dict) -> bool:
        ...
        policy = tool.get("approval", "never")
        if policy == "always":
            return True
    ...
    def _execute(self, tool: dict, arguments: dict, key, on_behalf_of: str, extra: dict, ref: str) -> dict:
        ...
        for attempt in range(1, allowed + 1):
            ...
            if latency <= limit:
                ...
                return {"status": "ok", "attempts": attempt, "elapsed_ms": elapsed, "log": log, "result": result}
            elapsed += limit
        ...
        return {"status": "timed_out", "attempts": allowed, "elapsed_ms": elapsed, "log": log, "result": None}
