...
contract path described in the capstone would have shown instead, under the
delivery-completeness rule stated in the pack README. Use it as
...
| 23:05 | Package fails at the load step; the on-failure retry re-sends the first North batch, which held inspection A, and moves on to East | nobody | — | The re-sent A is the same event delivered again; accepted state unchanged |
...
| 3 Mar 01:10 | Dashboard refresh runs on schedule over the partial reporting database: inspection A present twice in North, East showing 1 March rows | nobody | — | D1 closes with no completion marker, so North is held with the reason "incomplete delivery"; East: "no delivery for 2 March"; last verified snapshots shown and labelled stale |
| 06:40 | `QC_Corrections_20260303.csv` with A version 2 (12/1) lands on the share | quality clerk | — | Delivery D2 admitted and retained; North stays held because the package run carrying D1 has not completed (a per-plant completion marker would have let North publish 20/1 here) |
...
| 11:15 | DBA reruns the package by hand: inspection A re-sent once more (ev-late, its third copy), East rows arrive | DBA | — | Delivery D3 completes the run: ev-late agrees with history and is older than v2; North publishes 20/1 at 5.0% with `evidence_as_of` 11:15; East publishes E, F, G at 5/44 = 11.4% |
...
| 16:30 | Workbook shows 7.7% for North (three copies of A1 plus A2); sheet 10.6% | analyst | — | 5.0%, `evidence_as_of` 11:15 |
...
The engineer-hours per investigation and the expediting cost per unit come
from the fictional plant controller's statement; the $60 hourly rate is
borrowed from the sponsor's planning assumptions; the 5 hours on 3 March is
this timeline's own hypothetical. All are planning assumptions, not
measurements. Cinderline does not log how often the
...
Facts within the fiction: the package retried and re-sent inspection A; East was
...
problem; that East was the problem the meeting should have investigated
(G's 3 defects in 9 units is the fact; the judgement is Mara's and Leo's);
that the hand paste "fixed" the figure; that deleting rows made the numbers
right. Each needs its own evidence before it enters a design.
