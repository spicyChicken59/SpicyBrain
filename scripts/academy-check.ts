...
export async function checkCourse(write = false) {
  ...
  if (write) {
    ...
    await writeFile(
      join(root, "docs/academy/COVERAGE.md"),
      coverageMarkdown(report, contract),
    );
  }
  return report;
}
/** A readable summary of COVERAGE.json: totals against the floors, every module by track, and every finding. */
export function coverageMarkdown(
  report: {
    date: string;
    totals: Record<string, number | Record<string, number>>;
    modules: Record<string, Record<string, number | string>>;
    findings: Finding[];
  },
  contract: Contract,
) {
  const t = report.totals as Record<string, number> & {
    labsByClass: Record<string, number>;
  };
  ...
  const lines = [
    ...
    "",
    `Generated by \`node --import tsx scripts/academy-check.ts --write\` on ${report.date}; the same data is in [\`COVERAGE.json\`](COVERAGE.json). Counts are coverage, not quality: the builder editorial review is [\`EDITORIAL-REVIEW.md\`](EDITORIAL-REVIEW.md).`,
    "",
    `**Result: ${fails.length ? `FAIL (${fails.length} failing findings)` : "PASS"}**, with ${warns.length} warnings listed below.`,
    "",
    "| Measure | Count | Contract floor |",
    "|---|---:|---:|",
    `| Modules, in ${t.tracks} tracks | ${n(t.modules)} | 48 |`,
    `| Teaching beats | ${n(t.beats)} | 480 |`,
    `| Visual states | ${n(t.visualStates)} | — |`,
    `| Cards: core + extension | ${n(t.coreCards)} + ${n(t.extensionCards)} = ${n(t.coreCards + t.extensionCards)} | 672 |`,
    `| Beats with a Samajh analogy | ${n(t.samajh)} | — |`,
    `| Lessons | ${n(t.lessons)} | — |`,
    `| Practice scenarios (plus capstones) | ${n(t.scenarios)} (+ ${n(t.capstones)}) | 3 capstones |`,
    `| Labs: local-executed / tabletop / platform guide | ${n(t.labs)}: ${n(t.labsByClass["local-executed"])} / ${n(t.labsByClass.tabletop)} / ${n(t.labsByClass["platform-guide"])} | 24 |`,
    `| Field guides | ${n(t.guides)} | 32 |`,
    `| Case analyses / crosswalk rows | ${n(t.cases)} / ${n(t.crosswalk)} | 8 cases |`,
    `| Reviewed video placements / media decisions | ${n(t.media)} / ${n(t.mediaDecisions)} | — |`,
    "",
  ];
  for (const track of contract.tracks) {
    ...
    for (const cm of track.modules) {
      const m = report.modules[cm.label] ?? {};
      lines.push(
        `| ${cm.label} | \`${cm.moduleId}\` | ${cm.disposition} | ${n(m.beats)} | ${n(m.states)} | ${n(m.objectiveChecks)} | ${n(m.samajh)} | ${n(m.coreCards)} | ${n(m.extensionCards)} | ${n(m.sources)} | ${n(m.handbookWords)} |`,
      );
    }
    lines.push("");
  }
  ...
  for (const f of report.findings) {
    const key = `${f.level.toUpperCase()}: ${f.message}`;
    grouped.set(key, [...new Set([...(grouped.get(key) ?? []), f.where])]);
  }
  for (const [key, where] of grouped)
    lines.push(
      `- **${key}** (${where.length}): ${where.map((w) => `\`${w}\``).join(", ")}`,
    );
  lines.push("");
  return lines.join("\n");
}
